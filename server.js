...
// Trading settings - UPDATED FOR 1000% PROFIT
const MAX_CONCURRENT_TRADES = 50;        // 50 concurrent trades
const PROFIT_PERCENT = 1000;              // 1000% profit target (10x return)
const PROFIT_CHECK_INTERVAL = 1000;       // Check every 1 second (faster)
const AGGRESSIVE_TRADE_SCALING = true;    // Increases trade count to reach high targets
...
// ========== AGGRESSIVE TRADING ENGINE - 1000% PROFIT TARGET ==========
...
app.post('/api/start-trading', authenticate, async (req, res) => {
    try {
        ...
        // Start the trading loop immediately
...
async function startAggressiveTrading(sessionId) {
    ...
    // Process existing trades (check for completions)
    for (let i = 0; i < session.activeTrades.length; i++) {
        const trade = session.activeTrades[i];
        
        // Check if this trade is ready to be processed
        if (trade.status === 'BUY_ORDER_PLACED') {
            // For demo mode, simulate fill after 1 second (faster for high target)
            if (session.isDemo && Date.now() - trade.createdAt > 1000) {
                ...
                // Immediately place sell order with PROFIT_PERCENT% profit target
            ...
            // For demo mode, simulate sell fill after 1 second (faster for high target)
    ...
    // If target reached after trade completion, stop
    if (session.currentBalance >= session.targetAmount) {
        session.status = 'TARGET_REACHED';
    ...
    // AGGRESSIVE SCALING - More trades needed for high profit targets
    // Calculate required trades based on profit per trade
    const profitPerTrade = session.currentBalance * (PROFIT_PERCENT / 100);
    let requiredTrades = Math.ceil(remainingNeeded / profitPerTrade);
    requiredTrades = Math.min(requiredTrades, MAX_CONCURRENT_TRADES * 10); // Allow up to 500 trades total
    
    // Calculate dynamic trade size - MORE AGGRESSIVE as time runs out
    let tradeSizeMultiplier = 1;
    if (timeRemaining < 0.5) tradeSizeMultiplier = 10;
    else if (timeRemaining < 1) tradeSizeMultiplier = 7;
    else if (timeRemaining < 2) tradeSizeMultiplier = 5;
    else if (timeRemaining < 3) tradeSizeMultiplier = 3;
    else if (timeRemaining < 4) tradeSizeMultiplier = 2;
    
    // Calculate how many new trades to place (up to MAX_CONCURRENT_TRADES = 50)
    let tradesToPlace = Math.min(
        MAX_CONCURRENT_TRADES - session.activeTrades.length,
        Math.max(requiredTrades, Math.ceil(20 / timeRemaining))
    );
    tradesToPlace = Math.max(1, Math.min(MAX_CONCURRENT_TRADES, tradesToPlace));
    
    // Calculate investment per trade - MORE AGGRESSIVE for higher profit target
    let investmentPerTrade = Math.min(session.currentBalance * 0.2 * tradeSizeMultiplier, remainingNeeded / tradesToPlace);
    
    // Ensure minimum investment
    if (investmentPerTrade < 10) {
        investmentPerTrade = Math.min(session.currentBalance * 0.15, remainingNeeded);
        if (investmentPerTrade < 10) {
            // If still too small, reduce number of trades
            tradesToPlace = Math.max(1, Math.floor(tradesToPlace / 2));
            investmentPerTrade = Math.min(session.currentBalance * 0.2, remainingNeeded / tradesToPlace);
            if (investmentPerTrade < 10) {
    ...
    for (let i = 0; i < tradesToPlace; i++) {
        ...
        try {
            ...
            newTradesPlaced++;
            console.log(`📈 NEW BUY ORDER #${newTradesPlaced}: ${roundedQty} ${symbol} @ ${buyPrice} (${PROFIT_PERCENT}% profit target) | Active trades: ${session.activeTrades.length}/${MAX_CONCURRENT_TRADES} | Required trades for target: ~${requiredTrades}`);
...
app.post('/api/trade-status', authenticate, (req, res) => {
    ...
    // Calculate estimated trades needed
    const profitPerTrade = session.currentBalance * (PROFIT_PERCENT / 100);
    const remainingNeeded = session.targetAmount - session.currentBalance;
    const estimatedTradesNeeded = Math.ceil(remainingNeeded / profitPerTrade);
    ...
    res.json({
        ...
        estimatedTradesNeeded: estimatedTradesNeeded,
        maxConcurrentTrades: MAX_CONCURRENT_TRADES,
        profitTargetPercent: PROFIT_PERCENT
    });
});
...
app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log(`✅ AGGRESSIVE SCALING: Enabled for high profit targets`);
